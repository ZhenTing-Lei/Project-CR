...
class ClashRoyaleApp: #l'esistenza intera del codice
    ...
    def intros(self): #roba estetica
        ...
        def delete_intro():
            if self.deck == []:   #sta roba controlla che tu abbia un mazzo
                ...
                time.sleep(0) #ricorda di cambiare il tempo
                self.main()
        ...
        def deck_edit(): #sta roba ti crea il mazzo
            ...
            #aggiungere cambiare il mazzo quando avremo piu carte
    ...
    def main(self): #sta roba ti crea la finestra di gioco, prima includeva tutto e partiva il gioco da solo, ma per colpa di Gio, ora non è più così grande
        ...
        self.root.bind("<Key>", place_troop.placeTroop)
    ...
    def create_map(self): #sta roba ti crea la mappa
        ...
        self.canvas.create_text(235, 50, text=str("time    :"), fill="black", font=("arial", 12)) #testo sul canvas del t e un :

    class GamePlay:
        def __init__(self, deck, canvas, root): #questo implementa le variabili che ci servono dopo
            self.canvas = canvas
            self.root = root
            ...
            for line in range(10): # ciclo for per una parte dell'elisir in modalità  barra
                self.canvas.create_line(30*line, 600, 30*line, 650, fill="black")
            self.showElixirBar() # funzione per inizializarre l'elisir in modalità barra
            self.cardButtom() #crea i bottoni delle carte
            self.chosenCard = 1 #carta scelta
        ...
        def update_elixir(self): #questo serve per aggiornare l'elixir
            ...
            if self.elixir > 10:
                self.elixir = 10
            self.elixir_text = self.canvas.create_text(145, 660, text=str(self.elixir), fill="black", font=("Arial", 12))
        ...
        def showElixirBar(self): #questo serve per mostrare l'elisir in modalità barra
            try:
                ...
                pass
            self.elixirUI = self.canvas.create_rectangle(0, 600, 30*self.elixir, 650, fill="purple")
        ...
        #LE robe del place troop sono ancora da fixxare... ma non so come
        def placeTroop(self, event): #scegliere che truppa delle 4 piazzare
            self.spawn_troop(self.chosenCard)
        ...
        def chooseTroop(self, event): #scegliere che truppa delle 4 piazzare
            if event.keysym in ["1", "2", "3", "4"]:
                self.chosenCard = int(event.keysym)

        def changeCard(self, cardIndex):
            self.chosenCard = cardIndex

        def cardButtom(self):
            button1 = tk.Button(self.root, text="card1", bg="yellow", activebackground="yellow", command=lambda: self.changeCard(0))
            button2 = tk.Button(self.root, text="card2", bg="light blue", activebackground="blue", command=lambda: self.changeCard(1))
            button3 = tk.Button(self.root, text="card3", bg="grey", activebackground="grey", command=lambda: self.changeCard(2))
            button4 = tk.Button(self.root, text="card4", bg="green", activebackground="green", command=lambda: self.changeCard(3))

            self.canvas.create_window(50, 100, window=button1)
            self.canvas.create_window(100, 100, window=button2)
            self.canvas.create_window(150, 100, window=button3)
            self.canvas.create_window(200, 100, window=button4)
