#dopo circa 8 mesi e 10 giorni, si ricomicia ad lavorare su questo progetto, sperando di ottimizzarlo e aggiungere funzioni migliori
...
# I autori di clash royale python 1.1, compreso copi_lot
def credits():
    print("")
...
tanky_deck = ["knight_f","valkyrie_f","pekka_f","mini_pekka_f","lumberjack_unraged_f","elite_barbs_f","giant_f","miner_f"]
...
#card cycle, la parte più storta e inefficiente del programma (sto provando a fixxarlo)

def game_play(event):
    #salva linput nella variabile keysym
    keysym=event.keysym
    if keysym in [1,2,3,4]:
        func = tanky_deck.get(tanky_deck[keysym-1])
        func()
        tanky_deck.append(tanky_deck[keysym-1])
        tanky_deck.pop(keysym-1)
        print(tanky_deck[0],tanky_deck[1],tanky_deck[2],tanky_deck[3])


    # Per cambiare la posizione di evocazione
    if keysym in ["a","s","d","f"]:
        if event == "a":
            cordinate_number = 0
            canvas.create_oval(50, 335, 60, 325, fill="black")
            canvas.create_oval(250, 335, 240, 325, fill="white")
